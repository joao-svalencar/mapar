      ...
      while(Q2 == 'y'| Q2 == 'Y') #Does user want to preview another area?
      {
        ...
        if(as.numeric(Q1) %in% 1:length(map.ar)) #checking if area number are provided
        {
          if(prop==FALSE) #conditional to DO NOT plot species percentage
          {
            ...
            {
              graphics::plot(shp, main=paste('Area', gtools::mixedsort(unique(lsp[,2]))[as.numeric(Q1)], sep=' '), cex.main=1.3) #study area plot
              graphics::plot(map.ar[[as.numeric(Q1)]][map.ar[[as.numeric(Q1)]]$per>=propcut,], add=TRUE, col=grDevices::rgb(0,0,1, 0.3)) #plots selected area WITHOUT species percentages/cell; attention to propcut
              ...
              graphics::plot(shp, main=paste('Area', sort(unique(lsp[,2]))[as.numeric(Q1)], sep=' '), cex.main=1.3) #study area plot
              graphics::plot(map.ar[[as.numeric(Q1)]][map.ar[[as.numeric(Q1)]]$per>=propcut,], add=TRUE, col=grDevices::rgb(0,0,1, 0.3)) #plots selected area WITHOUT species percentages/cell; attention to propcut
              cat("\nSpecies composition of Area", gtools::mixedsort(unique(lsp[,2]))[as.numeric(Q1)], ":\n \n")
              print(rownames(areas[[as.numeric(Q1)]])) #preview species composition
            }
            ...
            {  
              graphics::plot(shp, main=paste('Area', gtools::mixedsort(unique(lsp[,2]))[as.numeric(Q1)], sep=' '), cex.main=1.3) #study area plot
              graphics::plot(map.ar[[as.numeric(Q1)]][map.ar[[as.numeric(Q1)]]$per>=propcut,], add=TRUE, col=grDevices::rgb(0,0,1, alpha=per[[as.numeric(Q1)]][match(map.ar[[as.numeric(Q1)]][map.ar[[as.numeric(Q1)]]$per>=propcut,]@data[["id"]], names(per[[as.numeric(Q1)]]))])) #plots selected area WITH species percentages/cell; attention to propcut
              cat("\nSpecies composition of Area", gtools::mixedsort(unique(lsp[,2]))[as.numeric(Q1)], ":\n \n")
              print(rownames(areas[[as.numeric(Q1)]])) #preview species composition
            }else{ #contional to "numeric" areas codes
              graphics::plot(shp, main=paste('Area', sort(unique(lsp[,2]))[as.numeric(Q1)], sep=' '), cex.main=1.3) #study area plot
